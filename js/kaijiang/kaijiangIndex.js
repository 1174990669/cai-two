...
kaijiangIndexObj.setDefConfig = function () {
    this.allLotteryType = ['ssq', 'dlt', 'jxk3','gxk3','jlk3','gx11x5','gd11x5','ftspf', 'spf14', 'd3', 'pl3', 'pl5']; // 开奖首页所有彩种，按显示顺序
};
...
kaijiangIndexObj.formatHtml = function (data) {
    ...
    for (var k in data) {
		...
		if(data[k]['lottery_no'].length == 11){
			data[k]['lottery_no'] = data[k]['lottery_no'].slice(2);
		}
        ...
        var contentStr = '';
        var linkStr = '<a data-t="goDetail" data-c="' + k + '"><span class="arrowbox"><em class="rtarrow icon"></em></span></a>';
        ...
        if (k == 'spf14') {
            ...
            contentStr = '<div class="clearfix">' + ballStr + '</div>'
        } else if (k == 'ftspf') {
