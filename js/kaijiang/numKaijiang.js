  ...
  var numKaijiangObj = new PageController({
	   'name': 'numKaijiang',
  ...
  numKaijiangObj.setDefConfig = function(){
    ...
    this.allLotteryType = ["ssq","dlt", "jxk3",'jlk3',"gx11x5","gd11x5","ftspf","spf14","d3","pl3","pl5"/*,"qxc","bsksf","gd11x5","hn4j1","lcbqc","scjq"*/];
...
//			console.log('开奖列表', msg);
  ...
  numKaijiangObj.setLotteryHtml = function(){
    switch(this.nowLotteryType){
	  ...
	  case "jlk3" : numKaijiangObj.setK3Lottery('jlk3');break;
  ...
  numKaijiangObj.setK3Lottery = function(){
    ...
    if(thisData == ''){
    	this.kjListObj.append('<div style="width:100%;text-align:center;color:#666;line-height: 100px;font-size:12px;">暂无开奖记录！</div>');
    	return false;
    }
    var html = '';
  ...
  numKaijiangObj.setSPF14Lottery = function(){
    ...
    if(!thisData || !thisData.length)return html.join("");
    for(var i=0,ilen=thisData.length;i<ilen;i++){
      html.push('<div class="kjli"><p class="mb15 clearfix"><span class="fl '+(i==0 && this.nowPage==0 ? 'fontblue' : '')+'">第'+thisData[i]['lotteryNum']+'期</span><span class="fr gray font12">'+thisData[i]['time']+'</span></p><div class="clearfix"><p class="clearfix">'+this.createLotteryHtml(thisData[i]['lotteryResult'])+'</p></div><span class="arrowbox" data-t="detailed" data-v="'+thisData[i]['lotteryNum']+'" ><em class="arrow"></em></span></div>');
    }
    if(this.nowPage == 0){
		this.kjListObj.html(html.join(""));
	}else{
